...
P, internIndex = adjustPhi_eqSys(phi)       # macierz skad co wlata/wylata
Pp = P[:, internIndex]                      # macierz bez war brzegowych
from scipy.sparse import csr_matrix
Mphi = Pp.dot(Pp.T)             # P * Ptrans
Mphi = csr_matrix(Mphi)         # przerob na sparse

def adjustPhiFlux():
    from scipy.sparse.linalg import cg
    Fphi = P.dot(phi.data)              # prawa strona P * Phi zero (z poprzedniego kroku)
    Lambda = cg(Mphi, Fphi)[0]          # rozwiaz ukl rownan za pommoca CG 1)
    dPhi = - Pp.T.dot(Lambda)           # wyzn poprawek na Phi 2)
    phi.data[internIndex] += dPhi       # dodaj poprawke do phi


adjustPhiFlux()

gradP = grad(p)   # gradient cisnienia z Green - Gaussa
...
for i in range(100):
    ...
    # A to wartosci wspolczynnikow dla punktow centralnyc komorek (diagonala)
    A = np.array(momX_M.diag)

    Hx = - momX_M.offdiagmul(Ux.data)           # offdiagnal to sasiedzi
    ...
    edgeU = EdgeField.vector(einterp(Ux), einterp(Uy))       # pole wektorowe predkosci [Ux, Uy] wyinterpolowanych wartosci na krawedzie (ze sr komurek  EdgeField.interp)
    phi = edgeU.dot(mesh.normals)                # phi = U * n

    Mpd, Fpd = laplace(1./A, p)

    # Compute the mass fluxes at the cells faces.
    Fpd = -Fpd + edgeDiv(phi)                   # edgeDiv = phi * dl_kraw = U * n * dl_kraw = strumien przez krawedz  RowCiaglosci
    ...
    # Green - Gauss  gradient cisnienia dla nowych poprawionych cisnien
    gradP = grad(p)

    # obliczone grad(p) do uaktualnienia pola predkosci
...
# animate_contour_plot([Uy.data.reshape((n,m))], dataRange=[0,1])
# plt.show()
#
#
